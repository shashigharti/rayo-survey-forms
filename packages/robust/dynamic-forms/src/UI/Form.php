...
class Form extends BaseUI
{
     ...
     * @var string
     */
    public $route_name = 'dynamic-forms';
...
//    ];
     ...
     */
    public $right_menu = [
        'add' => ['display_name' => 'Add', 'url' => 'admin.dynamic-forms.create', 'permission' => 'forms.form.add','icon' => 'add']
    ];
    ...
    {
        return $form->exists ? ['admin.dynamic-forms.update', $form->id] : ['admin.dynamic-forms.store'];
    }
    ...
    {
        return route('admin.dynamic-forms.show', [$id]);
    }
    ...
    {
        return "<a href='" . route('admin.dynamic-forms.show', [$row['id']]) . "'>" . $row['title'] . "</a>";
    ...
    {
        if (is_array($this->right_menu['add']['url'])) {
            $callback = $this->right_menu['add']['url']['callback'];
            return $this->$callback($this->params['form']->id);
        }

        return route($this->right_menu['add']['url']);
    }
